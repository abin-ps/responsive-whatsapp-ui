import 'dart:ui';

const backgroundColor = Color.fromRGBO(13, 20, 24, 1);
const textColor = Color.fromRGBO(241, 241, 242, 1);
const appBarColor = Color.fromRGBO(42, 47, 50, 1);
const webAppBarColor = Color.fromRGBO(42, 47, 50, 1);
const chatBarMessage = Color.fromRGBO(42, 47, 50, 1);
const messageColor = Color.fromRGBO(5, 97, 98, 1);
const senderMessageColor = Color.fromRGBO(50, 55, 57, 1);
const tabColor = Color.fromRGBO(5, 97, 98, 1);
const searchBarColor = Color.fromRGBO(50, 55, 57, 1);
const dividerColor = Color.fromRGBO(50, 55, 57, 1);
